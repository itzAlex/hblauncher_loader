...
Result http_download_payload(char *url, u32 *payloadsize)
{
	...
	if(statuscode!=200)
	{
		printf("Error: El servidor ha devuelto el siguiente código %u.\n", (unsigned int)statuscode);
		httpcCloseContext(&context);
		return -2;
	}
	...
	{
		printf("Inválido HTTP tamaño de contenido: 0x%08x.\n", (unsigned int)contentsize);
		ret = -3;
		httpcCloseContext(&context);
		return ret;
	}
...
Result loadsd_payload(char *filepath, u32 *payloadsize)
{
	...
	*payloadsize = filestats.st_size;
	...
	{
		printf("Tamaño inválido del payload de la SD: 0x%08x.\n", (unsigned int)filestats.st_size);
		return -3;
	}
...
Result load_hblauncher()
{
	...
	printf("Obteniendo versión de sistema/información de sistema etc...\n");
	...
	if(region>=7)
	{
		printf("El valor de la region en cfgu es inválido: 0x%02x.\n", (unsigned int)region);
		ret = -9;
		return ret;
	}
	cfguExit();
	...
	if(ret!=0)
	{
		printf("Fallo en conseguir la versión de sistema: 0x%08x.\n", (unsigned int)ret);
		return ret;
	}
	...
	{
		printf("Esquivando la carga del payload de la SD porque el bóton X esta presionado.\n");
		ret = 1;
	}

	if(ret==0)
	{
		printf("El otherapp payload para esta aplicación ya existe, va a ser usado en vez de descargarlo.\n");
		payload_src = 0;
	}
	else
	{
		...
		if(ret!=0)
		{
			...
			if(ret==0xd8a0a046)
			{
				printf("No hay conexión a Internet utilizable.\n");
			}
			else
			{
				printf("Si el server no esta caído, y la solicitud HTTP és actualmente correcta, esto puede significar que su sistema de versión o región no está soportado por el hblauncher-payload actualmente.\n");
			}
			return ret;
		}
	...
	{
		printf("Tamaño del payload invalido: 0x%08x.\n", (unsigned int)payloadsize);
		ret = -3;
		return ret;
	}
	...
	{
		ret = 0xfe;
		printf("Fallo en localizar el préambulo.\n");
		return ret;
	}
	...
	ret = 0xff;
	printf("El payload ha vuelto atrás en la app, esto *nunca* pasa con el actual hblauncher-payload.\n");
...
int main(int argc, char **argv)
{
	...
	printf("hblauncher_loader %s por yellows8. Traducido al español por @alexpui2002 \n", VERSION);

	ret = svcDuplicateHandle(&kproc_handledup, 0xffff8001);
	if(ret!=0)printf("svcDuplicateHandle() con el siguiente proc-handle falló: 0x%08x.\n", (unsigned int)ret);
	...
	if(ret==0)
	{
		for(pos=0; pos<PAYLOAD_TEXTMAXSIZE; pos+=0x1000)
		{
			...
			if(ret!=0)
			{
				printf("svcControlProcessMemory con pos=0x%x falló: 0x%08x.\n", (unsigned int)pos, (unsigned int)ret);
				break;
			}
		}
	}
	...
	if(ret!=0)
	{
		...
		if(ret==0xd8e06406)
		{
			printf("El servicio HTTPC és inaccesible.\n");
		}
	}
